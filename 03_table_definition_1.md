...
## 全体
- [fix]→直っていなかったので、下記再度指摘です。
- カラム名にテーブル名と同じワードを含めることはおすすめしません。
    - 例えばusersテーブルのuser_telカラムは、telで十分です。usersテーブルのデータなのでuserのtel情報だということはわかり切っているためです。また、コードを書く際、@user.user_telと書かなくてはいけなくなり、コードの可読性が下がることも考えられます。
